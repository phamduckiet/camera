@extends('client.share.master')
@section('content')
<div class="container">
    <div class="row">
        <div class="col-lg-5 offset-lg-1 col-md-12 order-1 order-lg-2">
            <div class="contact-page-side-content">
                <h3 class="contact-page-title">Contact Us</h3>
                <p class="contact-page-message mb-25">Claritas est etiam processus dynamicus, qui sequitur mutationem consuetudium lectorum. Mirum est notare quam littera gothica, quam nunc putamus parum claram anteposuerit litterarum formas human.</p>
                <div class="single-contact-block">
                    <h4><i class="fa fa-fax"></i> Address</h4>
                    <p>123 Main Street, Anytown, CA 12345 – USA</p>
                </div>
                <div class="single-contact-block">
                    <h4><i class="fa fa-phone"></i> Phone</h4>
                    <p>Mobile: (08) 123 456 789</p>
                    <p>Hotline: 1009 678 456</p>
                </div>
                <div class="single-contact-block last-child">
                    <h4><i class="fa fa-envelope-o"></i> Email</h4>
                    <p>yourmail@domain.com</p>
                    <p>support@hastech.company</p>
                </div>
            </div>
        </div>
        <div class="col-lg-6 col-md-12 order-2 order-lg-1">
            <div class="contact-form-content pt-sm-55 pt-xs-55">
                <h3 class="contact-page-title">Nhập thông tin</h3>
                <div class="contact-form">
                    <form action="{{Route('lapdat')}}" method="post">
                    @csrf
                        <div class="form-group">
                            <label>Tên: <span class="required">*</span></label>
                            <input name="ten" id="ten" type="text">
                        </div>

                        <div class="form-group">
                            <label>Số điện thoại:<span class="required">*</span></label>
                            <input name="std" id="sodienthoai"  type="text">
                        </div>
                        <div class="form-group mb-30">
                            <label>Lựa chọn:<span class="required">*</span></label>
                            <select class="nice-select wide" name="theloai" id="theloai" style="display: none;">
                                <option value=0 data-display=""></option>
                                <option value="1">Lắp đặt</option>
                                <option value="2">Sửa chữa</option>
                                <option value="3">Tư vấn</option>

                            </select>
                        </div>
                        <div class="form-group mb-30">
                            <label>Ghi chú</label>
                            <textarea name="ghichu" id="ghichu"></textarea>
                        </div>
                        <div class="form-group">
                            <button class="li-btn-3" type="submit" value="submit">send</button>
                        </div>
                    </form>
                    </div>
                <p class="form-messege"></p>
            </div>
        </div>
    </div>
</div>
@endsection
